...
class Frame(wx.Frame):
    ...
    def slideUpDown(self, up):
        ...
        for col in range(numCols):
            cvl = [self.data[row][col] for row in range(numRows) if self.data[row][col]]
            if len(cvl) >= 2:
    ...
    def slideLeftRight(self, left):
        ...
        for row in range(numRows):
            rvl = [self.data[row][col] for col in range(numCols) if self.data[row][col]]
            if len(rvl) >= 2:
    ...
    def doMove(self, move, score):
        if move:
            ...
            if self.isGameOver():
                if wx.MessageBox(u'Game Over. Retry?', u'-_-', wx.YES_NO | wx.ICON_INFORMATION) == wx.YES:
    ...
    def drawScore(self, dc):
        ...
        dc.DrawText(u'BEST', 505 - 15 - bestScoreBoardW + (bestScoreBoardW - bestScoreSize[0]) / 2, 48)
        ...
        dc.DrawText(str(self.curScore), 505 - 15 - bestScoreBoardW - 5 - curScoreBoardW + (curScoreBoardW - scoreLabelSize[0]) / 2, 68)
...
if __name__ == '__main__':
    app = wx.App()
    Frame(u'2048 v1.0.0 By furzoom')
    app.MainLoop()
